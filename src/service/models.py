...
class TeamMember(BaseModel):
    position = models.CharField(_('Позиция'), max_length=120)
    order = models.PositiveIntegerField(_('Порядок'), default=0, help_text=_('Позиция для сортировки'))
...
class Service(BaseModel):
    title = models.CharField(_('Заголовок'), max_length=120)
    short_description_for_banner = models.CharField(_('Краткое описание для баннера главной страницы'), help_text=_('Макс. символов: 120'), max_length=120)
    gif = models.FileField(_('Гифка'), upload_to='gifs/', validators=[FileExtensionValidator(allowed_extensions=['gif'], message=_('Допустимые форматы: .gif'))], help_text=_('Загрузите гифку для услуги. Допустимые форматы: .gif'))
    tabs = models.ManyToManyField(Tab, related_name='service_tabs', verbose_name=_('Табы'))
    order_service = models.PositiveIntegerField(_('Порядок'), default=0, help_text=_('Позиция для сортировки'))
    ...
    class Meta:
        ...
        ordering = ['order_service', 'created_at']
